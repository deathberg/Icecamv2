...
public final class VliveBinderClient {
    ...
    private int[] lastPollCounters = new int[5];
    ...
    public int[] lastPollCounters() { return Arrays.copyOf(lastPollCounters, lastPollCounters.length); }
    ...
    /** TX13 — pipeline heartbeat counters (5× int32), polled ~1 Hz by BinderStatePoller. */
    public int[] pollState() {
        ...
        try {
            ...
            if (b == null) {
                ...
                return Arrays.copyOf(lastPollCounters, lastPollCounters.length);
            }
            data.writeInterfaceToken(DESCRIPTOR);
            ...
            if (!Arrays.equals(counters, lastPollCounters)) {
                log.log("binder", String.format(java.util.Locale.US,
                        "TX13 counters=[%d,%d,%d,%d,%d]",
                        counters[0], counters[1], counters[2], counters[3], counters[4]));
            }
            lastPollCounters = counters;
            return counters;
        } catch (Throwable t) {
    ...
    public String diagnostics() {
        ...
        int[] poll = lastPollCounters();
        sb.append("lastPollCounters=[")
                .append(poll[0]).append(',').append(poll[1]).append(',').append(poll[2]).append(',')
                .append(poll[3]).append(',').append(poll[4]).append("]\n");
