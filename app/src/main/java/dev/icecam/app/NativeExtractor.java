...
public final class NativeExtractor {
    ...
    private static final String[] NATIVE_LIBS = {"libvc.so", "libshadowhook.so", "vcplax.so"};
        ...
        try {
            ...
            for (String n : NATIVE_LIBS) {
                ...
                sb.append("extracted ").append(entryName).append(" -> ").append(dst.getAbsolutePath())
                        .append(" size=").append(dst.length()).append('\n');
            }
            zip.close();
        } catch (Throwable t) {
            ok = false;
            sb.append("extract error: ").append(t).append('\n');
        }
